...
def next_run_dir(output_root: str | Path, dataset_name: str) -> Path:
    root = Path(output_root) / "softformer" / dataset_name
    ensure_dir(root)
...
def validate(model: nn.Module, loader: DataLoader, device: torch.device) -> float:
    ...
    total = 0
    with torch.no_grad():
        for hsi, aux, targets in loader:
            hsi = hsi.to(device)
            aux = aux.to(device)
            targets = targets.to(device)
            predicted = torch.argmax(model(hsi, aux), dim=1)
...
def test_labeled_samples(model: nn.Module, loader: DataLoader, device: torch.device, n_classes: int) -> dict:
    ...
    targets_all = []
    with torch.no_grad():
        for hsi, aux, targets in tqdm(loader, desc="test labeled samples"):
            hsi = hsi.to(device)
            aux = aux.to(device)
...
def train_one_run(
    ...
    model = SoftFormerBaseline(
        ...
        opt_chans=int(config.get("hsi_channels", 10)),
        sar_chans=int(config.get("aux_channels", 4)),
        num_classes=len(labels),
        stem_chans=int(config.get("stem_chans", 16)),
        embed_dim=list(config.get("embed_dim", [24, 48, 96])),
        num_heads=list(config.get("num_heads", [4, 8, 16])),
        mlp_ratio=float(config.get("mlp_ratio", 4.0)),
        depths=list(config.get("depths", [2, 8, 2])),
        drop=float(config.get("drop", 0.0)),
        attn_drop=float(config.get("attn_drop", 0.1)),
        drop_path_rate=float(config.get("drop_path_rate", 0.1)),
        use_isa=bool(config.get("use_isa", True)),
    ).to(device)
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(config["epochs"]), eta_min=float(config.get("eta_min", 0.0)))
    ...
    for epoch in tqdm(range(1, epochs + 1), desc=f"run {run_index}"):
        model.train()
        for hsi, aux, targets in train_loader:
            hsi = hsi.to(device)
            aux = aux.to(device)
            targets = targets.to(device)
            optimizer.zero_grad()
            loss = criterion(model(hsi, aux), targets)
            loss.backward()
            optimizer.step()
        ...
        if use_validation:
            ...
            test_metrics = test_labeled_samples(model, test_loader, device, len(labels))
...
def run_training(config_path: str | Path) -> Path:
    ...
    expected_channels = int(config.get("hsi_channels", 10)) + int(config.get("aux_channels", 4))
    if image.shape[-1] < expected_channels:
        raise ValueError(f"Expected at least {expected_channels} channels, got {image.shape[-1]}")
    if int(config["patch_size"]) < int(config.get("model_img_size", 8)):
        raise ValueError("SoftFormer patch_size must be >= model_img_size.")
