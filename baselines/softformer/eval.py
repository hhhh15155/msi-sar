...
def load_model(config: dict, labels: list[str], checkpoint: Path, device: torch.device) -> SoftFormerBaseline:
...
def save_labeled_maps(run_dir: str | Path, run_index: int = 0) -> dict:
    ...
    batch_size = int(eval_config.get("batch_size", config.get("batch_size", 64)))
