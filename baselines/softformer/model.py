...
class StemConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[-1] > self.img_size or x.shape[-2] > self.img_size:
            top = (x.shape[-2] - self.img_size) // 2
            left = (x.shape[-1] - self.img_size) // 2
...
class SoftFormerBaseline(nn.Module):
    def __init__(
        self,
        img_size=8,
        opt_chans=10,
        ...
        super().__init__()
        if img_size % 4 != 0:
            raise ValueError("SoftFormer model_img_size must be divisible by 4.")
        self.model = SoftFormer(
            img_size=img_size,
            opt_chans=opt_chans,
            sar_chans=sar_chans,
            num_class=num_classes,
            stem_chans=stem_chans,
            embed_dim=embed_dim or [24, 48, 96],
            num_heads=num_heads or [4, 8, 16],
            mlp_ratio=mlp_ratio,
            depths=depths or [2, 8, 2],
            drop=drop,
            attn_drop=attn_drop,
