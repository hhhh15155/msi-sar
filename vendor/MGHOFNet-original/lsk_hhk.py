# train_lsk_hhk.py - LSK-FusionNet (SS-LSK) Training for HHK Dataset
# 适配 HHK (8chengliu) 数据集 (6类)
...
PATCH_SIZE = 11
PCA_COMPONENTS = 30
TRAIN_SAMPLES_PER_CLASS = 10  # HHK 常用每类 50 或 150，可根据需求调整

# --- 路径设置 (参考 vsda_hhk.py) ---
# 确保 data/8chengliu/ 下有 8chengliu_HSI.mat, 8chengliu_LiDAR.mat, label.mat
SAVE_DIR_PARAMS = 'cls_params/lsk_hhk'
SAVE_DIR_RESULTS = 'cls_result/lsk_hhk'
SAVE_DIR_MAPS = 'cls_map/lsk_hhk'

# --- 模型结构参数 (HHK 6 Classes) ---
NUM_CLASSES = 6
EMB_DIM = 128
DEPTH = 2
...
def loadData():
    """加载 HHK (8chengliu) 数据集"""
    print("Loading HHK (8chengliu) dataset...")
    # 路径与 key 参考 vsda_hhk.py
...
def select_traintest(groundTruth, num_train_per_class):
    """
    均衡采样策略：每类随机选取固定数量 (用于小样本/均衡实验)
    """
    labels_loc = {}
    train = {}
    test = {}
    m = int(max(groundTruth))  # HHK 为 6
...
def main():
    print('=' * 80)
    print('LSK-FusionNet Training (HHK Dataset)')
    ...
    # 3. 构建模型 (Model from model.py)
    # 你的 model.py 已经集成了 SecondOrderPoolingHead
    ...
    for epoch in pbar:
        ...
        if (epoch + 1) % 10 == 0 or (epoch + 1) == EPOCHS:
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                torch.save(model.state_dict(), f'{SAVE_DIR_PARAMS}/LSKNet_HHK_best.pth')
    ...
    model.load_state_dict(torch.load(f'{SAVE_DIR_PARAMS}/LSKNet_HHK_best.pth'))
    y_pred, y_true = test(model, test_iter, device)

    # HHK 6 类 (参考 vsda_hhk.py)
    target_names = [
        'Spartina alterniflora',  # 互花米草
        'Phragmites australis',   # 芦苇
        'Water Body',             # 水体
        'Suaeda glauca',          # 碱蓬
        'Tamarix chinensis',      # 柽柳
        'Roads'                   # 道路
    ]
    ...
    # 写入结果文件
    with open(f'{SAVE_DIR_RESULTS}/LSKNet_HHK_report.txt', 'w') as f:
        f.write(f'HHK Dataset (6 Classes)\n')
        f.write(f'Train Samples/Class: {TRAIN_SAMPLES_PER_CLASS}\n')
        f.write(f'Model: SS-LSK (with 2nd-Order Head)\n\n')
        f.write(f'OA: {oa:.2f}%\nAA: {aa:.2f}%\nKappa: {kappa:.2f}%\n\n')
    ...
    if GENERATE_LABELED_MAP and total_iter is not None:
        ...
        Utils.generate_png(total_iter, model, y, device, total_indices, f'{SAVE_DIR_MAPS}/LSKNet_HHK_labeled')

    if GENERATE_ALL_MAP and all_iter is not None:
        print('\nGenerating full classification map...')
        Utils.generate_all_png(all_iter, model, y, device, all_indices, f'{SAVE_DIR_MAPS}/LSKNet_HHK_all')
