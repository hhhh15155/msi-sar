...
def load_mat_hsi(dataset_name, dataset_dir):
    ...
    elif (dataset_name == 'Ori'):
        image = hdf5storage.loadmat(os.path.join(dataset_dir, dataset_name, "data.mat"))
        image = image['data']
        gt = hdf5storage.loadmat(os.path.join(dataset_dir, dataset_name, "label.mat"))
        gt = gt['label']
        labels = [
            'Undefined',
            'shuiti',
